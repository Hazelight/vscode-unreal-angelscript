...
export function ParseModule(module : ASModule, debug : boolean = false)
{
    ...
    // Traverse syntax trees to lift out functions and variables during this first parse step
...
// Parse the specified module
export function LoadAndParseModule(module : ASModule)
{
    if (!module.loaded)
...
export function ResolveModule(module : ASModule)
{
    ...
    while (!module.resolved)
    {
        ...
        for (let dependencyModule of module.moduleDependencies)
        {
            ...
            {
                LoadAndParseModule(dependencyModule);
            ...
            {
                PostProcessModuleTypes(dependencyModule);
                module.resolved = false;
            }
        }
        ...
        else
        {
            ...
            if (potentialModules)
            {
                ...
                for (let moduleWithSuper of potentialModules)
                    LoadAndParseModule(moduleWithSuper);
...
export function GetModulesPotentiallyDependentOnSymbol(asmodule : ASModule, findSymbol : ASSemanticSymbol) : Array<ASModule>
{
    switch (findSymbol.type)
    {
        ...
        case ASSymbolType.TemplateBaseType:
        {
            ...
            if (dbtype)
            {
                ...
                if (dbtype.declaredModule)
                    return GetModulesPotentiallyDependentOn(dbtype.declaredModule);
                else
        ...
        case ASSymbolType.Namespace:
        {
            ...
            if (dbnamespace)
                return GetModulesPotentiallyDependentOnNamespace(dbnamespace);
        }
        break;
...
export function GetModulesPotentiallyDependentOn(findModule : string) : Array<ASModule>
{
...
export function GetModulesPotentiallyDependentOnMultiple(findModules : Array<string>) : Array<ASModule>
{
    let markedModules = new Set<string>();
...
export function GetModulesPotentiallyDependentOnNamespace(namespace : typedb.DBNamespace) : Array<ASModule>
{
    // Limit to things importing this type if it's a script type
    ...
    return GetModulesPotentiallyDependentOnMultiple(allDeclaredModules);
}
    ...
    while (anyMarked)
    {
        ...
        for (let module of ModuleDatabase.values())
        {
            ...
            else
            {
                ...
                // since it might use symbols from our module
                isPotentialUser = true;
            }

            // Mark the module if it depends on a marked module
            if (isPotentialUser)
            {
...
export function ResolveTypeFromExpression(scope : ASScope, node : any) : typedb.DBType
{
    ...
    switch (node.type)
    {
        ...
        case node_types.FunctionCall:
        {
            ...
            if (left_func.determinesOutputTypeArgumentIndex != -1
                && node.children[1] && node.children[1].children && node.children[1].children[left_func.determinesOutputTypeArgumentIndex])
            {
    ...
    switch (node.type)
    {
        ...
        case node_types.FunctionCall:
        {
            ...
            if (node.children[0] && node.children[0].type == node_types.Identifier)
            {
                ...
                {
                    left_type = refType;
                    AddIdentifierSymbol(scope, statement, node.children[0], ASSymbolType.Typename, null, refType.name);
            ...
            else if (left_symbol instanceof typedb.DBMethod && left_symbol.determinesOutputTypeArgumentIndex != -1)
            {
...
function DetectIdentifierSymbols(scope : ASScope, statement : ASStatement, node : any, parseContext : ASParseContext, symbol_type : typedb.DBAllowSymbol) : typedb.DBSymbol | typedb.DBType
{
    ...
    if (globalSymbols)
    {
        for (let usedSymbol of globalSymbols)
        {
            ...
            {
                AddIdentifierSymbol(scope, statement, node, ASSymbolType.GlobalVariable,
                    usedSymbol.namespace.getQualifiedNamespace(), usedSymbol.name, parseContext.isWriteAccess);
    ...
    if (symType)
    {
        AddIdentifierSymbol(scope, statement, node, ASSymbolType.Typename, null, symType.name);
    ...
    if (isTypingSingleIdentifier || node.maybeTypename)
    {
        ...
        if (nsType)
        {
            ...
            AddIdentifierSymbol(scope, statement, node, symType, null, nsType.getQualifiedNamespace());
...
function CheckIdentifierIsPrefixForValidSymbol(scope : ASScope, statement : ASStatement, parseContext : ASParseContext, identifierPrefix : string, symbol_type : typedb.DBAllowSymbol) : boolean
{
    ...
    // We could be typing a global variable
    {
    ...
    // We could be typing a global get accessor
    if (typedb.AllowsProperties(symbol_type))
    {
...
function DetectSymbolsInType(scope : ASScope, statement : ASStatement, inSymbol : typedb.DBType | typedb.DBSymbol, node : any, parseContext : ASParseContext, symbol_type : typedb.DBAllowSymbol) : typedb.DBSymbol | typedb.DBType
{
    ...
    if (typedb.AllowsFunctions(symbol_type))
    {
        // Check if this is a mixin call from a different module
        ...
        if (mixinFunctions)
        {
            for (let symbol of mixinFunctions)
            {
                if (symbol instanceof typedb.DBMethod)
                {
                    ...
                    if (symbol.args.length != 0 && dbtype.inheritsFrom(symbol.args[0].typename))
                    {
                        AddIdentifierSymbol(scope, statement, node, ASSymbolType.GlobalFunction, symbol.namespace.getQualifiedNamespace(), symbol.name);
...
function DetectSymbolsInNamespace(scope : ASScope, statement : ASStatement, namespace : typedb.DBNamespace, node : any, parseContext : ASParseContext, symbol_type : typedb.DBAllowSymbol) : typedb.DBSymbol | typedb.DBType
{
    ...
    if (usedSymbol)
    {
        ...
        {
            AddIdentifierSymbol(scope, statement, node, ASSymbolType.Typename, null, usedSymbol.name);
        ...
        else
        {
            ...
            AddIdentifierSymbol(scope, statement, node, symType, usedSymbol.namespace.getQualifiedNamespace(), usedSymbol.name);
            scope.module.markDependencySymbol(usedSymbol);
            return usedSymbol;
        }
    }
    ...
    if (typedb.AllowsProperties(symbol_type))
    {
        ...
        if (getAccessor && getAccessor instanceof typedb.DBMethod)
        {
            ...
            AddIdentifierSymbol(scope, statement, node, symType, getAccessor.namespace.getQualifiedNamespace(), getAccessor.name);
            scope.module.markDependencyFunction(getAccessor);
            return typedb.LookupType(getAccessor.namespace, getAccessor.returnType);
        }
        ...
        {
            symType = ASSymbolType.GlobalAccessor;
            AddIdentifierSymbol(scope, statement, node, symType, setAccessor.namespace.getQualifiedNamespace(), setAccessor.name);
            scope.module.markDependencyFunction(setAccessor);
            return typedb.LookupType(setAccessor.namespace, setAccessor.args[0].typename);
        }
    }
