...
class ASFile {
    ...
    GetLocationRange(startOffset, endOffset) {
        return {
            uri: this.pathname,
            range: {
...
;
var ASScopeType;
(function (ASScopeType) {
    ASScopeType[ASScopeType["Global"] = 0] = "Global";
    ASScopeType[ASScopeType["Class"] = 1] = "Class";
    ASScopeType[ASScopeType["Function"] = 2] = "Function";
    ASScopeType[ASScopeType["Enum"] = 3] = "Enum";
    ASScopeType[ASScopeType["Other"] = 4] = "Other";
    ASScopeType[ASScopeType["Namespace"] = 5] = "Namespace";
})(ASScopeType = exports.ASScopeType || (exports.ASScopeType = {}));
;
class ASVariable {
}
exports.ASVariable = ASVariable;
;
class ASScope {
    constructor() {
        ...
        this.content = "";
        this.unscopedContent = new Array();
        this.unscopedOffsets = new Array();
    }
    GetScopeAt(offset) {
    ...
    toDBType() {
        ...
        dbtype.properties = new Array();
        dbtype.methods = new Array();
...
function ParseDeclarations(root) {
...
function RemoveScopeFromDatabase(scope) {
...
exports.RemoveScopeFromDatabase = RemoveScopeFromDatabase;
let loadedFiles = new Map();
let filesByModuleName = new Map();
function GetFile(pathname) {
    return loadedFiles.get(decodeURIComponent(pathname));
}
exports.GetFile = GetFile;
function GetFileByModuleName(modulename) {
    return filesByModuleName.get(modulename);
}
exports.GetFileByModuleName = GetFileByModuleName;
function UpdateContent(pathname, modulename, content, document) {
    ...
    if (document)
        ...
        file.document = vscode_languageserver_1.TextDocument.create(pathname, "angelscript", 1, content);
...
function CreatePosition(pathname, offset) {
...
function ResolvePosition(pathname, pos) {
    let doc = GetDocument(pathname);
    if (doc == null)
        return -1;
...
exports.GetSymbolLocationInScope = GetSymbolLocationInScope;
function _GetScopeSymbol(file, scope, symbolname) {
    // Find variables
...
exports.GetTypeSymbolLocation = GetTypeSymbolLocation;
function PostProcessScope(scope) {
...
function PostProcessModule(modulename) {
    let file = GetFileByModuleName(modulename);
    if (!file)
        return;
    PostProcessScope(file.rootscope);
}
exports.PostProcessModule = PostProcessModule;
//# sourceMappingURL=as_file.js.map